...
language sql security definer set search_path='' stable as $function$
 ...
 exposure as(select hm.business_id id,count(pd.id)::bigint n from public.promotion_deliveries pd join public.hive_campaigns hc on hc.id=pd.campaign_id join public.hive_members hm on hm.hive_id=hc.hive_id and hm.status='active' and hm.business_id<>pd.source_business_id,cutoff c where pd.created_at>=c.ts and pd.status in('sent','delivered') and hm.business_id in(select id from visible) group by hm.business_id),
