...
begin
 ...
 if p_channel='email' and not coalesce(l.marketing_email_allowed,false) then raise exception 'email_consent_unavailable'; end if;
 if p_channel='sms' and not coalesce(l.marketing_sms_allowed,false) then raise exception 'sms_consent_unavailable'; end if;
