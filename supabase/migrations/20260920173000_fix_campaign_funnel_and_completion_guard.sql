...
returns jsonb language sql security invoker set search_path='' stable as $function$
 ...
 del as(select count(*)::bigint delivery_total,count(*) filter(where status='queued')::bigint queued,count(*) filter(where status='sending')::bigint sending,count(*) filter(where status='sent')::bigint sent,count(*) filter(where status='delivered')::bigint delivered,count(*) filter(where status='failed')::bigint failed from d),
...
begin
 ...
 if p_status in('scheduled','active') then
  ...
  perform public.refresh_hive_campaign_audiences(p_campaign_id);perform public.snapshot_hive_campaign_recipients(p_campaign_id);
 end if;
