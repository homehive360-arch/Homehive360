...
begin
 ...
 if not ((d.status='sending' and p_status in ('sent','failed')) or (d.status='sent' and p_status in ('delivered','failed'))) then raise exception 'invalid_delivery_transition'; end if;
 ...
 if d.provider_message_id is not null and p_provider_message_id is not null and nullif(btrim(p_provider_message_id),'') is distinct from d.provider_message_id then raise exception 'provider_message_id_mismatch'; end if;
