-- Replace legacy referral naming in Hive operational reporting.
drop function if exists public.hive_operational_health(integer);

create function public.hive_operational_health(p_days integer default 30)
returns table(
 id uuid,
 name text,
...
as $function$
 ...
 select vh.id,vh.name,
  ...
  coalesce(a.opportunities,0),coalesce(a.awaiting,0),coalesce(a.overdue,0),
