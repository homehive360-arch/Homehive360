...
export async function sendSms(input:SmsInput):Promise<SendResult>{
 ...
 try{const body=new URLSearchParams({To:input.to,From:from,Body:input.text});const r=await fetch('https://api.twilio.com/2010-04-01/Accounts/'+encodeURIComponent(sid)+'/Messages.json',{method:'POST',headers:{Authorization:'Basic '+Buffer.from(sid+':'+token).toString('base64'),'Content-Type':'application/x-www-form-urlencoded'},body});const data=await r.json().catch(()=>({}));if(!r.ok)return{ok:false,error:String(data?.message||'SMS provider rejected request')};const providerMessageId=typeof data?.sid==='string'?data.sid.trim():'';if(!providerMessageId)return{ok:false,error:'SMS provider response missing message ID'};return{ok:true,providerMessageId};}catch{return{ok:false,error:'SMS provider unavailable'};}
}
