...
export async function POST(request:NextRequest){
 ...
 if(body.offerId){const offer=(await db.from('offers').select('id').eq('id',body.offerId).eq('business_id',business.id).eq('status','active').or('hive_id.eq.'+hive.id+',hive_id.is.null').maybeSingle()).data;if(!offer)return NextResponse.json({error:'Invalid offer'},{status:422});}
 ...
 let opportunityId:string|null=null;if(delivery){opportunityId=(await db.from('opportunities').select('id').eq('lead_id',delivery.lead_id).eq('receiving_business_id',business.id).maybeSingle()).data?.id||null;}
 ...
 const result=await db.rpc('record_public_attribution_event',{p_event_key:eventKey,p_hive_id:hive.id,p_business_id:business.id,p_customer_id:delivery?.customer_id||null,p_lead_id:delivery?.lead_id||null,p_event_type:eventType,p_properties:{delivery_id:delivery?.id||null,source_business_id:sourceId,source_slug:body.sourceSlug||null,target_business_id:business.id,opportunity_id:opportunityId,offer_id:body.offerId||null,channel:'hive_page'}});
 if(result.error)return NextResponse.json({error:'Event not recorded'},{status:500});return NextResponse.json({ok:true,attributed:Boolean(delivery),opportunity_id:opportunityId,recorded:Boolean(result.data)});
}