...
export async function POST(request:NextRequest){
 ...
 const source=(await db.from('businesses').select('name,slug,status').eq('id',lead.source_business_id).maybeSingle()).data;
 const hive=(await db.from('hives').select('slug,market_name,status').eq('id',lead.hive_id).maybeSingle()).data;
