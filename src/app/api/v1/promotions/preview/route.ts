...
export async function POST(request:NextRequest){
 ...
 const [customerResult,sourceResult,hiveResult]=await Promise.all([db.from('customers').select('first_name,email,phone').eq('id',lead.customer_id).maybeSingle(),db.from('businesses').select('name,slug,status').eq('id',lead.source_business_id).maybeSingle(),db.from('hives').select('slug,market_name,status').eq('id',lead.hive_id).maybeSingle()]);if(customerResult.error||sourceResult.error||hiveResult.error)return NextResponse.json({error:'Promotion context lookup failed'},{status:500});const customer=customerResult.data,source=sourceResult.data,hive=hiveResult.data;
 if(!customer||!source||!hive)return NextResponse.json({error:'Promotion context incomplete'},{status:409});if(source.status!=='active'||hive.status!=='active')return NextResponse.json({error:'Promotion source or Hive is not active'},{status:409});
