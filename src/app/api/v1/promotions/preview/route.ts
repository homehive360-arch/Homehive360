...
export async function POST(request:NextRequest){
 ...
 if(!customer||!source||!hive)return NextResponse.json({error:'Promotion context incomplete'},{status:409});if(customer.source_business_id!==lead.source_business_id)return NextResponse.json({error:'Promotion customer does not belong to source business'},{status:409});if(source.status!=='active'||hive.status!=='active')return NextResponse.json({error:'Promotion source or Hive is not active'},{status:409});
