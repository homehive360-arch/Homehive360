...
export default function ForgotPassword(){
 ...
 async function submit(e:FormEvent){
  e.preventDefault();setMsg('Sending reset link...');
  const db=createBrowserSupabase();
  const configuredOrigin=(process.env.NEXT_PUBLIC_APP_URL||'').replace(/\/$/,'');
  const redirectTo=(configuredOrigin||window.location.origin)+'/auth/reset';
  const {error}=await db.auth.resetPasswordForEmail(email,{redirectTo});
  setMsg(error?error.message:'If an account exists for that email, check your inbox for a password reset link.');
 }
